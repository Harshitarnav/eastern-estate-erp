...
exports.generateUserSeedSQL = exports.UserSeeder = void 0;
    ...
    async run() {
        ...
        try {
            ...
            for (const role of roles) {
                const result = await queryRunner.query(`INSERT INTO roles (name, display_name, description, is_system, is_active) 
           VALUES ($1, $2, $3, $4, $5) 
           ON CONFLICT (name) DO UPDATE SET 
             display_name = $2, description = $3 
           RETURNING id`, [role.name, role.displayName, role.description, role.isSystem, true]);
            ...
            for (const permission of permissions) {
                const result = await queryRunner.query(`INSERT INTO permissions (name, display_name, module, description) 
           VALUES ($1, $2, $3, $4) 
           ON CONFLICT (name) DO UPDATE SET 
             display_name = $2, module = $3, description = $4 
           RETURNING id`, [permission.name, permission.displayName, permission.module, permission.description]);
            ...
            for (const [roleName, permissionNames] of Object.entries(rolePermissions)) {
               ...
               VALUES ($1, $2) 
               ON CONFLICT DO NOTHING`, [roleIds[roleName], permissionIds[permName]]);
                    }
                }
            ...
            for (const user of users) {
                const existingUser = await queryRunner.query(`SELECT id FROM users WHERE email = $1 OR username = $2`, [user.email, user.username]);
                let userId;
             ...
             WHERE id = $7`, [defaultPassword, user.firstName, user.lastName, user.phone, true, true, userId]);
                    console.log(`✅ Updated user: ${user.email} (${user.role})`);
                }
                else {
                    const userResult = await queryRunner.query(`INSERT INTO users (email, username, password_hash, first_name, last_name, phone, is_active, is_verified, email_verified_at) 
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW()) 
             RETURNING id`, [user.email, user.username, defaultPassword, user.firstName, user.lastName, user.phone, true, true]);
                    userId = userResult[0].id;
                    console.log(`✅ Created user: ${user.email} (${user.role})`);
                }
                await queryRunner.query(`INSERT INTO user_roles (user_id, role_id) 
           VALUES ($1, $2) 
           ON CONFLICT DO NOTHING`, [userId, roleIds[user.role]]);
            }
...
exports.generateUserSeedSQL = generateUserSeedSQL;
