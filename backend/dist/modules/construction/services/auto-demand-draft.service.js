...
let AutoDemandDraftService = AutoDemandDraftService_1 = class AutoDemandDraftService {
    constructor(demandDraftRepository, paymentScheduleRepository, progressRepository, flatRepository, customerRepository, bookingRepository, propertyRepository, towerRepository, milestoneDetectionService, flatPaymentPlanService, templateService, settingsService, mailService, autoSendResolver) {
        ...
        this.autoSendResolver = autoSendResolver;
    ...
    async autoGenerateDemandDrafts() {
        ...
        try {
            ...
            this.logger.log(`Auto sweep complete: ${created} draft(s) created, ${skipped} already existed`);
        }
        catch (error) {
            this.logger.error('Error during auto demand draft generation:', error);
        }
    }
    async generateDemandDraftSafe(match) {
    ...
    async generateDemandDraft(match, systemUserId) {
        ...
        const autoSend = await this.autoSendResolver.resolve(flatPaymentPlan.customerId, flat.property?.id ?? null);
        ...
        const demandDraft = this.demandDraftRepository.create({
            ...
            title: draftTitle,
            amount,
            status: autoSend.shouldAutoSend ? demand_draft_entity_1.DemandDraftStatus.SENT : demand_draft_entity_1.DemandDraftStatus.DRAFT,
            content: htmlContent,
            dueDate,
            metadata: {
                subject,
            ...
            tone: demand_draft_entity_1.DemandDraftTone.ON_TIME,
            reminderCount: 0,
            escalationLevel: 0,
            daysOverdue: 0,
            nextReminderDueAt,
        ...
        if (autoSend.shouldAutoSend && customer.email) {
            try {
                ...
                if (result.skipped) {
                    this.logger.warn(`Auto-send email skipped (SMTP not configured) for draft ${savedDemandDraft.id}`);
                }
                else {
                    this.logger.log(`Auto-sent DD ${savedDemandDraft.id} emailed to ${customer.email} (source=${autoSend.source})`);
                }
            }
            catch (mailErr) {
                this.logger.error(`Auto-send email failed for draft ${savedDemandDraft.id}: ${mailErr?.message}`);
            }
        }
        else if (autoSend.shouldAutoSend && !customer.email) {
            this.logger.warn(`Auto-send requested for draft ${savedDemandDraft.id} but customer has no email - marked SENT without delivery`);
        }
...
], AutoDemandDraftService.prototype, "autoGenerateDemandDrafts", null);
