/**
 * Default HTML templates seeded into `demand_draft_templates` on boot,
 * one per tone. Admins can edit these via the DD templates page; these
 * files only run if a row for that tone does not already exist.
 *
 * All templates share the same placeholder vocabulary so the scanner and
 * generators can reuse the same templateData shape:
 *   {{customerName}} {{propertyName}} {{towerName}} {{flatNumber}}
 *   {{milestoneName}} {{milestoneDescription}} {{amount}} {{dueDate}}
 *   {{dateIssued}} {{refNumber}} {{bookingNumber}} {{daysOverdue}}
 *   {{totalAmount}} {{paidAmount}} {{balanceAfterPayment}}
...
export const DEFAULT_TONE_HTML: Record<string, string> = {
  ON_TIME:
    LETTERHEAD.replace('{{documentTitle}}', 'Payment Demand Notice') +
  ...
  REMINDER_1:
    LETTERHEAD.replace('{{documentTitle}}', 'Gentle Reminder - Payment Pending') +
  ...
  REMINDER_2:
    LETTERHEAD.replace('{{documentTitle}}', 'Important: Payment Overdue') +
  ...
  REMINDER_3:
    LETTERHEAD.replace('{{documentTitle}}', 'FINAL NOTICE - Payment Overdue') +
  ...
  REMINDER_4:
    LETTERHEAD.replace('{{documentTitle}}', 'LAST CHANCE - Clear Your Payment') +
  ...
  CANCELLATION_WARNING:
    LETTERHEAD.replace('{{documentTitle}}', 'NOTICE OF CANCELLATION WARNING') +
  ...
  POST_WARNING:
    LETTERHEAD.replace('{{documentTitle}}', 'Continued Payment Default') +
