...
export class ConstructionSchemaSyncService implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    try {
      ...
      // ── Reconcile legacy columns on construction_flat_progress ──────────────
      //   Older installs carry NOT NULL columns the current entity no longer
      //   populates (project_id, progress_date, etc.). Backfill the known pairs
      //   and drop NOT NULL from every known legacy column so fresh inserts
      //   that only write the entity's columns succeed. Idempotent.
