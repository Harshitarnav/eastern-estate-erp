...
export const marketingService = {
  async getCampaigns(filters: CampaignFilters = {}): Promise<PaginatedCampaignResponse> {
    const response = await axios.get(`${API_URL}/marketing/campaigns`, { params: filters });
  ...
  async updateMetrics(id: string, metrics: any): Promise<Campaign> {
    const response = await axios.patch(`${API_URL}/marketing/campaigns/${id}/metrics`, metrics);
    return response.data;
  },
  ...
  async getStatistics(): Promise<any> {
    const response = await axios.get(`${API_URL}/marketing/campaigns/statistics`);
    return response.data;
  },
};
